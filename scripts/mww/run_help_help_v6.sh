...
KEYWORD_ID="help_help_v6"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== help help v6 MWW training ==="
...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/en_helphelp_adv_v6/en_helphelp_adv_v6/training/en_helphelp_adv_v6_mmap"
if [ -d "$ADV_MMAP" ]; then
    log "Adversarial mmap exists, skip"
else
    log "Generating adversarial mmap..."
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        --input-dir /workspace/outputs/help_help_adversarial_v6 \
        --output-dir ${DATA_DIR}/negative_datasets/en_helphelp_adv_v6/en_helphelp_adv_v6 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/help_help_v6" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/help_help_v6
features:
...
- features_dir: negative_datasets/no_speech/no_speech
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/en_helphelp_adv_v6/en_helphelp_adv_v6
...
TEST_DIR="/workspace/outputs/help_help_edgetts_v6/test"
