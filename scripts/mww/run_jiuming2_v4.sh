...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/jiuming2_adv_v4/jiuming2_adv_v4/training/jiuming2_adv_v4_mmap"
if [ -d "$ADV_MMAP" ]; then
    ...
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        ...
        --output-dir ${DATA_DIR}/negative_datasets/jiuming2_adv_v4/jiuming2_adv_v4 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/jiuming2_v4" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/jiuming2_v4
features:
...
- features_dir: negative_datasets/zh_chinese/zh_chinese
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/jiuming2_adv_v4/jiuming2_adv_v4
