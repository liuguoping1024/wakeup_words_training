...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/en_helphelp_adv_v3/en_helphelp_adv_v3/training/en_helphelp_adv_v3_mmap"
if [ -d "$ADV_MMAP" ]; then
    ...
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        ...
        --output-dir ${DATA_DIR}/negative_datasets/en_helphelp_adv_v3/en_helphelp_adv_v3 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/help_help_v3" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/help_help_v3
features:
...
- features_dir: negative_datasets/no_speech/no_speech
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/en_helphelp_adv_v3/en_helphelp_adv_v3
