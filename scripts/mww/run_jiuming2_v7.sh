...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/jiuming2_adv_v6/jiuming2_adv_v6/training/jiuming2_adv_v6_mmap"
if [ -d "$ADV_MMAP" ]; then
    ...
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        ...
        --output-dir ${DATA_DIR}/negative_datasets/jiuming2_adv_v6/jiuming2_adv_v6 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/jiuming2_v7" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/jiuming2_v7
features:
...
- features_dir: negative_datasets/zh_chinese/zh_chinese
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/jiuming2_adv_v6/jiuming2_adv_v6
...
# 验证
log "Verifying..."
TEST_DIR="/workspace/outputs/jiuming2_edgetts_v6/test"
