#!/usr/bin/env python3
"""
help help v4 edge-tts 生成 — 严格正样本 + 扩展负样本。
"""
...
async def generate_batch(texts, voices, output_dir, prefix, repeat=1):
    """repeat: 每个 text×voice 组合生成几次"""
...
async def main():
    base_dir = sys.argv[1] if len(sys.argv) > 1 else "outputs/help_help_edgetts_v4"

    # 正样本：3 词 × 22 声音 × 10 repeat = 660 条
    print(f"=== 正样本（{len(POSITIVE_PHRASES)} 词 × {len(EN_VOICES)} 声音 × 10 repeat）===")
    pos = await generate_batch(POSITIVE_PHRASES, EN_VOICES,
                               os.path.join(base_dir, "positive"), "pos", repeat=10)

    # 负样本：~120 词 × 22 声音 = 2640 条
    print(f"\n=== 负样本（{len(NEGATIVE_PHRASES)} 词 × {len(EN_VOICES)} 声音）===")
    neg = await generate_batch(NEGATIVE_PHRASES, EN_VOICES,
                               os.path.join(base_dir, "negative"), "neg", repeat=1)
    ...
    test_pos = await generate_batch(
        ...
        os.path.join(base_dir, "test"), "tpos", repeat=2)
    test_neg = await generate_batch(
        ...
        test_voices, os.path.join(base_dir, "test"), "tneg", repeat=1)
