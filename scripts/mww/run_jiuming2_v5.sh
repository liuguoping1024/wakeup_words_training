...
KEYWORD_ID="jiuming2_v5"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== 救命救命 v4 MWW training ==="
...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/jiuming2_adv_v5/jiuming2_adv_v5/training/jiuming2_adv_v5_mmap"
if [ -d "$ADV_MMAP" ]; then
    log "Adversarial mmap exists, skip"
else
    log "Generating adversarial mmap..."
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        --input-dir /workspace/outputs/jiuming2_adversarial_v4 \
        --output-dir ${DATA_DIR}/negative_datasets/jiuming2_adv_v5/jiuming2_adv_v5 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/jiuming2_v5" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/jiuming2_v5
features:
...
- features_dir: negative_datasets/zh_chinese/zh_chinese
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/jiuming2_adv_v5/jiuming2_adv_v5
