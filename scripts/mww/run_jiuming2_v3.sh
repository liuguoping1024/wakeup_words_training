...
KEYWORD_ID="jiuming2_v3"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== 救命救命 v3 MWW training ==="
...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/jiuming2_adv_v3/jiuming2_adv_v3/training/jiuming2_adv_v3_mmap"
if [ -d "$ADV_MMAP" ]; then
    log "Adversarial mmap exists, skip"
else
    log "Generating adversarial mmap..."
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        --input-dir /workspace/outputs/jiuming2_adversarial_v3 \
        --output-dir ${DATA_DIR}/negative_datasets/jiuming2_adv_v3/jiuming2_adv_v3 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/jiuming2_v3" \
    --data-dir "${DATA_DIR}" \
...
window_step_ms: 10
train_dir: trained_models/jiuming2_v3
features:
...
- features_dir: negative_datasets/zh_chinese/zh_chinese
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/jiuming2_adv_v3/jiuming2_adv_v3
