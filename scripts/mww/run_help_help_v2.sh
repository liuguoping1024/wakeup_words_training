...
KEYWORD_ID="help_help_v2"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

log "=== help help v2 MWW training ==="
...
# 对抗性负样本 mmap
ADV_MMAP="${DATA_DIR}/negative_datasets/en_helphelp_adv_v2/en_helphelp_adv_v2/training/en_helphelp_adv_v2_mmap"
if [ -d "$ADV_MMAP" ]; then
    log "Adversarial mmap exists, skip"
else
    log "Generating adversarial mmap..."
    python3 ${SCRIPT_DIR}/generate_adversarial_features.py \
        --input-dir /workspace/outputs/help_help_adversarial_v2 \
        --output-dir ${DATA_DIR}/negative_datasets/en_helphelp_adv_v2/en_helphelp_adv_v2 \
        --data-dir ${DATA_DIR}
fi
...
python3 ${SCRIPT_DIR}/03_generate_features_real.py \
    --positive-dir "${DATA_DIR}/positive_augmented/help_help_v2" \
    --data-dir "${DATA_DIR}" \
...
# 训练配置 — v2 改进：对抗性权重 15/2.0，30000 步，+ MUSAN 背景
log "Writing training config..."
cat > "${MWW_DIR}/training_parameters.yaml" <<'YAMLEOF'
window_step_ms: 10
train_dir: trained_models/help_help_v2
features:
...
- features_dir: negative_datasets/no_speech/no_speech
  ...
  truncation_strategy: random
  type: mmap
- features_dir: negative_datasets/en_helphelp_adv_v2/en_helphelp_adv_v2
...
TEST_DIR="/workspace/outputs/help_help_edgetts_v2/test"
...
log "Verifying with real voices..."
